...
turn_off_light(clientNum,light_num,dont_kill_threads)
{
	...
	if(!isDefined(level._box_locations[light_num])){
		IPrintLnBold( light_num );
		return;
	}
	name = level._box_locations[light_num];
...
//	PrintLn("**** TBM - got " + state + " for client " + clientNum);
	...
	{	
		turn_off_this = false;
		if(s < 0){
			if(abs(s) >= level._box_locations.size){
				turn_off_this = true;
				s = int(abs(s));
			}else{
				return;
		...
		turn_off_all = true;
		if(s >= (level._box_locations.size*2))					//changed for mod
		{
			if (s < 200 || s >= (level._box_locations.size*2)+200 ){
				return;
			}else{
				s-= 200;
				turn_off_all = false;
		...
		bright = true;
		if(s >= level._box_locations.size){
			s -= level._box_locations.size;
			bright = false;
		}
		...
		if(turn_off_all){
			...
			turn_off_light(clientNum,level._box_indicator);
		}
												//changed for mod
		...
		if(turn_off_this){
			...
			if(level._power_on)			// changed for mod
			{
				if(bright)
					turn_light_green(clientNum,level._box_indicator, true);
				else
					turn_light_green(clientNum,level._box_indicator, false);
			}
		}
	}
}
