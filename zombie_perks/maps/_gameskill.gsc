...
playerHealthRegen()
{
	...
	for( ;; )
	{
		...
		{
			if( gettime() - hurttime < get_playerHealth_RegularRegenDelay())					//CHANGED FOR MOD
			{
				continue;
			}
			...
			if( veryHurt )
			{
				...
				if( gettime() > hurtTime + GetLongRegenTime())									//CHANGED FOR MOD
				{
					newHealth += GetRegenRate(regenRate);										//CHANGED FOR MOD
				}
...
HasQuickReviveBuff(){
	return is_true(self.QUICKREVIVE_INCREASED_REGEN) || (level.QUICKREVIVE_INCREASED_REGEN && self maps\_zombiemode_perks::HasThePerk("specialty_quickrevive"));
}
GetRegenRate(regenRate)
{							//REGEN RATE WHEN AT LOW HP
	if(!is_true(level.zombiemode))
		return regenRate;

	if(self HasQuickReviveBuff())
		return regenrate *1.5;
	else
		//return regenrate/max(1,level.round_number/5) ;
		return regenrate;
}
get_playerHealth_RegularRegenDelay(){	//DELAY TO REGEN HEALTH 
	if(!is_true(level.zombiemode))
		return level.playerHealth_RegularRegenDelay;

	if(self HasQuickReviveBuff())
		return level.playerHealth_RegularRegenDelay*0.7;
	else
		//return level.playerHealth_RegularRegenDelay*1.5;
		return level.playerHealth_RegularRegenDelay;
}

GetOverLayFadeOverTime(){
	if(!is_true(level.zombiemode))
		return 3.5;
	...
	if(self HasQuickReviveBuff())
		...
		//return 3.5/max(1,level.round_number/5) ;
		return 3.5;
}

GetLongRegenTime(){
	if(!isdefined( level.zombiemode ) || !level.zombiemode)
		return level.longRegenTime;

	if(self HasQuickReviveBuff())
		return level.longRegenTime;
	else
		return level.longRegenTime/max(1,level.round_number/5) ;
}
...
shouldShowCoverWarning()
{
	...
	if ( !is_true(maps\_load_common::map_is_early_in_the_game()))		//added "is_true" EDITED FOR MOD
	{
		return false;
	}
...
redFlashingOverlay( overlay )
{
	...
	stopFlashingBadlyTime = gettime() + GetLongRegenTime(); 		//CHANGED FOR MOD
...
healthOverlay_remove( overlay )
{
	...
	if ( GetDvar( #"zombiemode" ) == "1" )
	{
		overlay fadeOverTime(GetOverLayFadeOverTime());
		overlay.alpha = 0;
	}
	else
	{
