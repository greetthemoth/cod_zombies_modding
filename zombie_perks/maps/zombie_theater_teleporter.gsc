...
teleporter_init()
{
	...
	//IPrintLnBold( "RAN TELEPORT INIT" );
...
teleporter_wait_to_turn_off(index){
	while(1){
		...
		//trigger maps\zombie_theater_magic_box::turnLightOff("teleporter_indicator_light1", true );
...
teleport_core_think( index )
{
	...
	if ( isdefined( trigger ) )
	{
		...
		while(1){
			//if(flag("power_on")){
			//IPrintLnBold( "times_cycled: "+times_cycled + " power_on:" + level.power_on);
			if(level.power_on){
...
teleport_core_is_on(trigger, index){
