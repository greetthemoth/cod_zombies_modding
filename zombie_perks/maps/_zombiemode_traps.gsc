...
{	
	...
	while(1)
	{
		...
		if(!level.power_on && trap.is_power_based)
			continue;
...
trap_move_switches()
{
	...
	self._trap_switches[0] waittill( "rotatedone" );
	if(level.power_on && self.is_power_based)
		self trap_lights_green();
}
...
trap_activate_electric()
{
	...
	self thread wait_trap_duration();

	self waittill_either( "trap_done","trap_duration_done" );
	// Shut down
	self notify ("trap_done");
...
wait_trap_duration(){
	...
	self notify("trap_duration_done");
}
