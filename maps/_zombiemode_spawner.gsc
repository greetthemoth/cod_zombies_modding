...
/*manageKillNow(){
	...
	while(1){
		level waittill_either( "door_closed", "end_of_round" );
		level.pf_failed_zombie_kill = false;
	}
}*/
...
/*killNow(){ 
	...
	notDeffed = !IsDefined(level.pf_failed_zombie_kill);
	if(notDeffed || !level.pf_failed_zombie_kill){
		//if(notDeffed){
		//	thread manageKillNow(); 		// thread keeps checks for doors to close and rounds in order tor reque for a barrier crosser..
		//}
		return false;
	}else{
		return true;
...
set_run_speed()
{
	...
	if(IsDefined( ZHC_zombie_move_speed_spike_chance ) && IsDefined( ZHC_zombie_move_speed_spike )){
		...
		if(level.ZHC_zombie_move_speed_spike_queue >= 100){
			...
			//else
			//	IPrintLnBold( "speed spike zombie. Speed: " + rand );
...
zombie_assure_node()
{
	...
	if(IsDefined(self.entrance_nodes))
	{
		for( i = 0; i < self.entrance_nodes.size; i++ )
		{
			...
			{
				//IPrintLn( "entrance node:" + i +"/"+ self.entrance_nodes.size + " bad path" );
	...
	IPrintLnBold( "BAD PATH KILL WAIT TO SECONDS" );
	wait(20);
...
head_should_gib(amount, attacker, type, point ) //changed for mod . added amount (damage)
{
	if ( !is_mature() )
	{
		return false;
	...
	if( weapon == "none"  ||
	// weapon == "m1911_zm" ||				//removed for mod
	WeaponIsGasWeapon( self.weapon ) )
	{
		return false; 
	}

	if(weapon == "m1911_zm"){ //added for mod
		percentToPass = 45;
		...
		if( damage_percent < percentToPass /*|| damage_percent >= 100*/ )
		{
			return false; 
		}
		return true;
...
//	self endon( "death" ); 
	...
	while( 1 )
	{
		...
		if( self head_should_gib(amount, attacker, type, point ) && type != "MOD_BURNED" )
		{
...
// 			else 
	...
	percentToPass = 10;												//added for mod
	if( type == "MOD_PISTOL_BULLET" || type == "MOD_RIFLE_BULLET" ) //changed for mod
	{
...
//	println( "**DEBUG self.health = ", self.health );
	...
	if( damage_percent < percentToPass /*|| damage_percent >= 100*/ )    //changed for mod . percent to pass used to be constant 10
	{
		return false; 
	}
...
zombie_pathing()
{
	...
	if(pfkill){
		self maps\ZHC_zombiemode_zhc::pathfinding_kill();
	}
