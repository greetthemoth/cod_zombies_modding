...
/*theater_playanim_reversed( animname )
{
	self UseAnimTree(#animtree);
	self animscripted(animname + "_done", self.origin, self.angles, level.scr_anim[animname],"normal", undefined, -2.0 );
}*/
...
electric_switch()
{
	...
	while(1){	//while added for mod
		...
		if(!level.power_on){
			...
			level.power_on = false;
			flag_clear( "power_on" );
			flag_set( "power_back_off" );
			//Objective_State(8,"done");
			
		}
		trig trigger_off();
		if(!power_able_to_turn_back_off)
			break;
