...
main()
{
	...
	//init_additionalprimaryweapon_machine_locations(); moved for mod to line 110 because wast running due to map info not being set yet
	...
	level.zhc_starting_weapon_slots = 2;
	level.secret_health_reducer_var = 0;
	level.zombie_damage = 45;
	level.secret_health_increaser_var = 0;
	...
	init_additionalprimaryweapon_machine_locations(); //moved for mod from line 17 
...
init_strings()
{
	...
	//ADDED FOR MOD
	add_zombie_hint( "default_buy_door_2250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_2250" );
	add_zombie_hint( "default_buy_door_2500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_2500" );
	add_zombie_hint( "default_buy_door_2750", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_2750" );
	add_zombie_hint( "default_buy_door_3000", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_3000" );
	add_zombie_hint( "default_buy_door_3250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_3250" );
	add_zombie_hint( "default_buy_door_3500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_3500" );
	add_zombie_hint( "default_buy_door_3750", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_3750" );
	add_zombie_hint( "default_buy_door_4000", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_4000" );
	add_zombie_hint( "default_buy_door_4250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_4250" );
	add_zombie_hint( "default_buy_door_4500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_4500" );
	add_zombie_hint( "default_buy_door_4750", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_4750" );
	add_zombie_hint( "default_buy_door_5000", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_5000" );
	add_zombie_hint( "default_buy_door_5250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_5250" );
	add_zombie_hint( "default_buy_door_5500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_5500" );
	add_zombie_hint( "default_buy_door_5750", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_5750" );
	add_zombie_hint( "default_buy_door_6000", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_6000" );
	add_zombie_hint( "default_buy_door_6250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_6250" );
	add_zombie_hint( "default_buy_door_6500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_6500" );
	add_zombie_hint( "default_buy_door_6750", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_6750" );
	add_zombie_hint( "default_buy_door_7000", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_7000" );
	add_zombie_hint( "default_buy_door_7250", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_7250" );
	add_zombie_hint( "default_buy_door_7500", &"ZOMBIE_BUTTON_BUY_OPEN_DOOR_7500" );
	//
...
//	level.crawlers_enabled			= 1;
	...
	// AI
	//zhc note: for whatever reason chaging modifers here has no effect
...
init_flags()
{
	...
	flag_init( "power_on" );
	flag_init( "power_back_off" );		//ADDED FOR MOD
	level.power_on = false; 			//ADDED FOR MOD
...
take_additionalprimaryweapon(additional_weapons)
{
	if(!isDefined(additional_weapons))
		additional_weapons = 0;
	...
	if(level.PERK_LEVELS){
		...
		for(i = 0; i<weaponsAmountToTake; i++){
			weapon_to_take = primary_weapons_that_can_be_taken[(primary_weapons_that_can_be_taken.size - 1 )- i];
			if ( weapon_to_take == self GetCurrentWeapon() )
			{
				retUndefined = weapon_to_take;
		...
		if(isDefined(retUndefined))
			return retUndefined;
		return weapons_taken[0];
	}else
...
//////////////////////
	/*if(level.round_number > 24){
		//max += level.round_number - 30;
		level.zombie_ai_limit = level.round_number;
		if(level.zombie_ai_limit > 40)
			...
			if ( spawn_dog )
			{
				...
				for ( i=0; i<keys.size; i++ )
				{
					if ( level.zones[ keys[i] ].is_occupied )
					{
						...
						akeys = array_randomize( akeys);								//				Added for mod . for more variability. makes dogs sapwn in random adjacent zone.
...
{
	level.zombie_health = level.zombie_vars["zombie_health_start"]; 
	for ( i=2; i<=round_number - level.secret_health_reducer_var; i++ )
	{
		// After round 10, get exponentially harder
		if( i >= 10 )
		{
			//CHANGED FOR MOD becaus seting var doesnt work
			//level.zombie_health += Int( level.zombie_health * level.zombie_vars["zombie_health_increase_multiplier"] ); 
			level.zombie_health += Int( level.zombie_health * 0.04 ); 
		}
		else
		{
			//CHANGED FOR MOD becaus seting var doesnt work
			//level.zombie_health = Int( level.zombie_health + level.zombie_vars["zombie_health_increase"] ); 
			level.zombie_health = Int( level.zombie_health + 50 ); 
		}
...
#/
	...
	else
	{
		...
		{
			//PrintLn("waiting... ec:"+get_enemy_count() + "  zt:" + level.zombie_total);
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	if( isDefined( eAttacker ) )
	{
		...
		if( (isDefined( eAttacker.is_zombie ) && eAttacker.is_zombie) || level.mutators["mutator_friendlyFire"] )
		{
			...
			{
				iDamage = level.zombie_damage;		// 45
			}
		}
...
actor_killed_override(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime)
{
	...
	////////////////////////////////// CHANGED FOR MOD
	self maps\ZHC_zombiemode_zhc::kill( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime );
	///////////////////////////////////
