...
#/
	...
	while( 1 )
	{
		...
		else if ( powerup == "full_ammo" )	//max ammos dont drop from enemies 		//ADDED FOR MOD
		{
			powerup = get_next_powerup();
		}
...
toggle_fire_sale_on()
{
	...
	if( level.zombie_vars["zombie_powerup_fire_sale_on"] )
	{
		for( i = 0; i < level.chests.size; i++ )
		{
			...
			{	
				create_temp = maps\_zombiemode_weapons::ZHC_FIRESALE_CAN_CREATE_TEMP_CHESTS() && !maps\_zombiemode_weapons::ZHC_chest_is_active(i);	//added by mod
				level.chests[i].zombie_cost = 10;
				if(!is_true(level.chests[i]._box_open) && create_temp)			//added for mod
		...
		for( i = 0; i < level.chests.size; i++ )
		{
			...
			if(show_firesale_box)
			{
				if(maps\_zombiemode_weapons::ZHC_FIRESALE_CAN_CREATE_TEMP_CHESTS() && !maps\_zombiemode_weapons::ZHC_chest_is_active(i) && IsDefined(level.chests[i].was_temp))
				{
					//level.chests[i].was_temp = undefined;		// moved for mod into func
					level thread remove_temp_chest( i );
				}
				
				if(IsDefined(level.chests[i].grab_weapon_hint) && (level.chests[i].grab_weapon_hint == true)) 
				//grab_weapon_hint turns on in middle box logic and turns off after middle box logic. og var not zhc
				{
...
{
	while( isdefined( level.chests[chest_index].chest_user ) || is_true(level.chests[chest_index]._box_open))
	{
		wait_network_frame();
	}
	level.chests[chest_index].was_temp = undefined;		// moved for mod
	level.chests[chest_index].premature_firesale_end = undefined;		// addded for mod
