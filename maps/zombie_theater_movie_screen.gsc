...
setupCurtains()
{
	...
	while (1){
		...
		if(!level.power_on){
			...
			//evel thread moveCurtains("right_curtain");
			...
			level thread raise_movie_screen();
		}
	}
...
moveCurtains(curtent, open)
{
	...
	if(!isdefined(curtain)){
		IPrintLn( "CURTAIN DOES NOT EXIST" );
		return;
	}
	curtorg = curtain.origin;
	time = 2; 	
	curtain thread monitorCurtain(curtorg,open);
...
monitorCurtain(curtorg,open)
{
	self endon("electricity_on");
	self endon("electricity_off");
